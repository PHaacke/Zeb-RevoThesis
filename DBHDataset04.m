...
%% Import the data
[~, ~, raw] = xlsread('/Users/Debora/Desktop/Peter/AdamBridgeDBH.xlsx','AdamTrialDBH');
raw = raw(2:end,:);
...
% intialize generic variables
 ...
 tString = ['DBH correlation for Dataset 4'];
 figure('Name', tString)
 hold on
 plot(xdata, ydata, 'ko') % Plot a scatter plot
 plot(fit1,fit2,'g','linestyle',':','LineWidth',1.5)
 plot (linePointsX,linePointsY,'-r','LineWidth',1.5) % Plot best fit line
 hold off
 legend({'Adam Bridge (n = 63)','1:1 line',['Y',' = ',num2str(pPoly(1)),'*',...
